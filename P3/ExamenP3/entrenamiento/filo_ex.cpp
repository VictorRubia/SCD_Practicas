...
void funcion_camarero(){
   ...
   while(true){
      ...
      {
      case etiq_sentarse:
         cout << "Camarero: El número de sillas ocupadas hasta el momento es " << filosofos_sentados << endl << flush;
         if(estado.MPI_SOURCE == 6){
             filosofos_sentados = filosofos_sentados + 2;
         }
         else
            filosofos_sentados++;
      ...
      case etiq_levantarse:
         filosofos_sentados--;
         if(lleno){
             cout << "Estaban cuatro filosofos sentados y se ha levantado el filosofo " << estado.MPI_SOURCE << endl << flush;
             lleno = false;
         }
         else
