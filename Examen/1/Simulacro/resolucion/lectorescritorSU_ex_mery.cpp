...
void revisar(){
    ...
    chrono::milliseconds dur_lectura(aleatorio<100,400>());

    mtx.lock();
    cout << "La revision esta siendo realizada..." << endl;
    mtx.unlock();
...
class LectoresEscritores: public HoareMonitor{
    private:
        int num_lec, num_esc;
        bool escribiendo, primera_vez;

        CondVar lectura, escritura, revisora;
    ...
    public:
        ...
        void fin_revision(){
            ...
            mtx.lock();
            cout << "Se terminó la revisión" << endl;
            mtx.unlock();
...
void funcion_hebra_revisora(MRef<LectoresEscritores> monitor){
    while(true){
        Espera();
